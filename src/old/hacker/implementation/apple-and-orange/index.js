...
function countApplesAndOranges(s, t, a, b, apples, oranges) {
  const isInRange = (position) => position >= s && position <= t;

  const appleCount = apples.filter((apple) => isInRange(apple + a)).length;
  const orangeCount = oranges.filter((orange) => isInRange(orange + b)).length;
